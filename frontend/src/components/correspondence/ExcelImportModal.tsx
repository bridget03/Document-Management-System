...
export default function ExcelImportModal({ open, onClose, direction, dirLabel, onImported }: Props) {
  ...
  const [result, setResult] = useState<{ total: number; success: number; failed: number; errors: { row: number; errors: string[] }[] } | null>(null);
  const { data: types } = useQuery<DocType[]>({ queryKey: ['corr-types-active'], queryFn: () => listDocTypes(true), enabled: open });
  ...
  const pickFile = async (f: File | undefined) => {
    ...
    try {
      ...
      const parsed: ParsedRow[] = json.map((raw, i) => {
        ...
        const typeId = resolveType((data as Record<string, unknown>).document_type);
        ...
        if ((data as Record<string, unknown>).document_type && !typeId)
          errors.push(`Loại văn bản '${String((data as Record<string, unknown>).document_type)}' không tồn tại.`);
  ...
  const confirm = async () => {
    ...
    try {
      ...
      setPhase('result');
      onImported();
      if (res.failed === 0) toast('success', `Import thành công ${res.success} văn bản.`);
      else toast('error', `Import xong: ${res.success} thành công, ${res.failed} thất bại.`);
    } catch (e: unknown) {
  ...
  const downloadErrors = () => {
    if (!result) return;
    const errMap = new Map(result.errors.map((e) => [e.row, e.errors.join('; ')]));
    const ws = XLSX.utils.json_to_sheet(
      rows.filter((r) => errMap.has(r.index)).map((r) => ({
        'Dòng': r.index,
      ...
      footer={
        phase === 'preview' ? (
          <>
            ...
            <Button variant="primary" loading={importing} disabled={validCount === 0} onClick={confirm}>
              Import {validCount} dòng hợp lệ
            </Button>
          </>
        ) : phase === 'result' ? (
          <>
            {result && result.failed > 0 && <Button onClick={downloadErrors}>Download error report</Button>}
    ...
    >
      {phase === 'pick' && (
        <div className="space-y-3">
          <label className="block cursor-pointer rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 p-8 text-center hover:border-gray-400">
            ...
            <p className="mt-2 text-sm font-medium text-gray-700">Kéo thả file Excel vào đây hoặc chọn file</p>
            <input
              type="file"
              accept=".xlsx,.xls"
              className="sr-only"
      ...
      {phase === 'preview' && (
        <div className="space-y-3">
          <p className="text-sm text-gray-600">
            Đã đọc: <b>{rows.length}</b> dòng · Hợp lệ: <b className="text-green-700">{validCount}</b> · Lỗi:{' '}
            <b className="text-red-600">{rows.length - validCount}</b> <span className="text-gray-400">({fileName})</span>
          </p>
          ...
          <button onClick={reset} className="flex items-center gap-1 text-xs text-gray-500 underline">
      ...
      {phase === 'result' && result && (
        <div className="space-y-2 text-sm">
          ...
          <p className="text-gray-600">Tổng số: {result.total} · Thành công: <span className="font-medium text-green-700">{result.success}</span> · Thất bại: <span className="font-medium text-red-600">{result.failed}</span></p>
          {result.errors.slice(0, 10).map((e) => (
            <p key={e.row} className="text-xs text-red-600">Dòng {e.row}: {e.errors.join('; ')}</p>
          ))}
          {result.errors.length > 10 && <p className="text-xs text-gray-500">... và {result.errors.length - 10} lỗi khác (xem error report).</p>}
        </div>
      )}
    </Modal>
  );

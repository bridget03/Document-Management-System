...
import { getDocument, downloadViaBlob, updateDocument } from '../services/documentApi';
...
export default function DocumentDetail() {
  ...
  const { data, isLoading, isError } = useQuery<Document>({ queryKey: ['doc', id], queryFn: () => getDocument(id!) });
  ...
  const save = useMutation({
    ...
    onError: (e: unknown) => {
      ...
      toast('error', typeof detail === 'string' ? detail : 'Could not save. You may lack permission.');
    },
  });
  ...
  return (
    <div className="space-y-4">
      <Link to="/documents" className="inline-flex items-center gap-1 text-sm font-medium text-gray-500 hover:text-gray-900">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h1 className="truncate text-xl font-bold text-gray-900">{data.name}</h1>
        ...
        <div className="flex flex-wrap gap-2">
          <Link to={`/documents/${data.id}/preview`}>
            <Button variant="primary"><Eye size={15} /> Preview</Button>
          </Link>
          ...
          {data.google_drive_url && (
            <a href={data.google_drive_url} target="_blank" rel="noreferrer">
              <Button><ExternalLink size={15} /> Open in Drive</Button>
            </a>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 xl:grid-cols-3">
        <Card className="space-y-4 p-5 xl:col-span-2">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">{data.description || <span className="text-gray-400">No description.</span>}</p>
          </div>
          <div className="grid grid-cols-1 gap-4 border-t border-gray-100 pt-4 sm:grid-cols-2">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-600">{data.category?.name || '—'}</p>
            </div>
            <div>
          ...
          <div className="border-t border-gray-100 pt-3">
            <Button size="sm" onClick={() => setEditing((e) => !e)}>
              <Pencil size={14} /> {editing ? 'Cancel editing' : 'Edit metadata'}
            </Button>
          </div>
          {editing && (
            <div className="space-y-3 rounded-lg bg-gray-50 p-4">
              ...
              <Field label="Tags (comma separated)">
                <TextInput value={tags} onChange={(e) => setTags(e.target.value)} placeholder="hop-dong, 2026" />
              </Field>
        ...
        <Card className="h-fit">
          <h2 className="border-b border-gray-200 px-5 py-3.5 text-sm font-semibold text-gray-900">Document info</h2>
          <div className="divide-y divide-gray-100 px-5">
            <InfoRow label="Status"><StatusBadge status={data.sync_status} source={data.source} /></InfoRow>
            <InfoRow label="Source">{data.source === 'GOOGLE_DRIVE' ? 'Google Drive' : 'Local upload'}</InfoRow>
            <InfoRow label="Created">{new Date(data.created_at).toLocaleString()}</InfoRow>
            <InfoRow label="Modified">{new Date(data.updated_at).toLocaleString()}</InfoRow>
          </div>
        </Card>
      </div>
    </div>

...
def audit(db: Session, user: User, action: str, doc_id: str | None, ip: str = "", ua: str = ""):
    db.add(AuditLog(user_id=user.id, action=action, document_id=doc_id, ip_address=ip, user_agent=ua))
...
def update_document(doc_id: str, payload: DocumentUpdate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    doc = db.query(Document).filter(Document.id == doc_id).first()
    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")
...
def preview_document(doc_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    filesystem paths or Google tokens."""
